...
set -uo pipefail
cd "$(dirname "$0")/.."   # droid-sim-evals root (this script lives in data/); paths below are relative to it
...
systemd-run --user --unit=tamp-datagen --collect --working-directory="$PWD" \
  ...
  bash -c ".venv/bin/python -u data/tamp_data_gen.py --num-envs 128 --num-tiptop-servers 1 --max-attempts 3600 --out-dir '$OUT' >> '$LOG' 2>&1"
